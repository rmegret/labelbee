...
@login_required
def videos_page():
    return render_template('pages/videos_page.html')
...
def version_page():
    ...
    label = subprocess.check_output(
        "git describe --always".split()).strip().decode()
    log = subprocess.check_output("git log -n 5".split()).strip().decode()
    hash = subprocess.check_output(shlex.split(
        "git log --pretty=format:'%H' -n 1")).strip().decode()
    date = subprocess.check_output(shlex.split(
        "git log --pretty=format:'%ci (%cr)' -n 1")).strip().decode()
...
def ajaxlogin():
    ...
    if user is None or not check_password(user, password):
        return jsonify({"request": "login", "email": email, "status": "FAIL", "message": "Invalid username or password"})
...
def parse_trackfilename(filename):
    ...
    m = re.search(
        r'(?P<video>C\d\d_\d{12})-Tracks-20(?P<timestamp1>\d{6})_(?P<timestamp2>\d{6}) - (?P<name>.*)\.json', filename)
    if (m is not None):
        d = m.groupdict()
...
def serve_files(base_dir, path, base_uri, format='html'):
    ...
    if not os.path.exists(filepath):
        raise BadRequest(
            'GET '+base_uri+': File not found "{}"'.format(filepath))
    ...
    if (format == 'html'):
        ...
        if (os.path.isfile(filepath)):
            raise BadRequest(
                'GET '+base_uri+': Attempting to get file in JSON format. try format=html')
        if (os.path.isdir(filepath)):
            ...
            result = {'files': files_obj, 'dirs': dirs_obj,
                      'base_uri': base_uri, 'path': path, 'uri': uripath}
...
def keypointlabels_get(path=''):
    ...
    base_dir = os.path.join(
        app.root_path, 'static/data/config/keypointlabels/')
...
def events_get_list():
    ...
    if (format == 'html'):
        ...
        for item in uri_list:
            string += '<button onclick="jsonFromServer(' + "'" + \
                item['uri'] + "'" + ')">' + item['filename'] + '</button> <br>'
        #print (string)
        result = {'data': string}
    elif (format == 'json'):
        result = uri_list
        ...
        raise BadRequest(
            '/rest/events GET: unrecognized format "{}"'.format(format))
