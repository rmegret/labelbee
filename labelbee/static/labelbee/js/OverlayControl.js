...
function OverlayControl(canvasTagId) {
  if (this === window) {
    console.log(
      'ERROR: OverlayControl should be created with "new OverlayControl()". Fixing that for you.'
    );
  ...
  $(videoControl).on("videosize:changed", (e, w, h) => {
  ...
  if (typeof canvasTagId === "undefined") canvasTagId = "canvas"; // Default HTML5 canvas tag to attach to
  ...
  excludeRects = [
    { cx: 335, cy: 75, w: 50, h: 50 },
    { cx: 2165, cy: 80, w: 50, h: 50 },
    { cx: 320, cy: 1034, w: 50, h: 50 },
    { cx: 2130, cy: 1080, w: 50, h: 50 },
  ];
...
OverlayControl.prototype.onClickAngleFromTag = function () {
  ...
  if (tag == null) {
    console.log(
      "onClickAngleFromTag: tag not defined for current event. abort."
    );
    return;
  }
...
OverlayControl.prototype.canvasSetVideoSize = function (w, h) {
  ...
  $("#canvasresize")[0].style.width =
    (wd + 16 + borderThickness).toString() + "px";
...
OverlayControl.prototype.refreshCanvasSize = function (event, ui) {
  ...
  if (logging.canvasEvents) {
    ...
    console.log(
      "refreshCanvasSize: event.target.clientWidth=",
  ...
  function resizeCanvas(wd, hd) {
    ...
    $("#canvasresize")[0].style.width =
      (wd + 16 + borderThickness).toString() + "px";
    $("#canvasresize")[0].style.height =
      (hd + borderThickness).toString() + "px";
  }
  ...
  $("#videoSize")[0].innerHTML =
    "videoSize: " +
    overlay.videoSize.w.toString() +
    "x" +
    overlay.videoSize.h.toString();
  $("#canvasSize")[0].innerHTML =
    "canvasSize: " + wd.toString() + "x" + hd.toString();
...
OverlayControl.prototype.canvasTransform_Fix = function () {
  ...
  if (hasNan || canvasTransform[0] <= 0 || canvasTransform[3] <= 0) {
    console.log(
      "ERROR in canvasTransform_Fix: degenerate canvasTransform. Reinit"
    );
    overlay.canvasTransformInternalReset();
  }
};
OverlayControl.prototype.canvasTransformScale = function (
  scaling,
  center,
  center2
) {
  ...
  if (typeof center2 === "undefined") center2 = center;
...
OverlayControl.prototype.redrawVideoFrame = function () {
  ...
  if (!videoControl.isValidVideo) {
    if (logging.frameEvents) console.log("refresh canceled, no valid video");
    overlay.ctx.save();
  ...
  if (videoControl.currentMode == "video") {
    ...
    if (videoControl.flagCopyVideoToCanvas) {
      ...
      overlay.ctx.clearRect(
        0,
        0,
        (video.videoWidth * extraScale) / transformFactor,
        (video.videoHeight * extraScale) / transformFactor
      );
    }
    ...
    overlay.ctx.drawImage(
      previewVideo,
      canvasTransform[4] / previewScaleX,
      canvasTransform[5] / previewScaleY,
      (canvasTransform[0] * w) / previewScaleX,
      (canvasTransform[3] * h) / previewScaleY,
      0,
      0,
      w,
      h
...
OverlayControl.prototype.refreshOverlay = function () {
  ...
  if (overlay.canvas1) {
    if (showTagsTracks || showSelectedTagsTracks) plotTagsTracks(overlay.ctx);
    if (showTags) plotTags(overlay.ctx);
...
function updateRectFromObsGeometry(rect) {
  if (logging.overlay) console.log("updateRectFromObsGeometry: rect=", rect);
  let obs = rect.obs;
...
function createRectsFromTracks(F) {
  if (typeof F === "undefined") F = getCurrentFrame();
  let ids = getValidIDsForFrame(F);
  if (logging.overlay) {
    console.log("createRectsFromTracks: ", { frame: F, ids: ids });
  }
  for (let id of ids) {
    // For each valid bee ID, create a rect for it
...
function addRectFromObs(inputObs, status) {
  ...
  if (typeof status == "undefined") status = "db";
  ...
  if (logging.addRect) console.log("addRectFromObs: added rect=", rect);
...
function rotatedRectGeometry(rect) {
  ...
  if (typeof rect.angle !== "undefined") geom.angle = rect.angle;
  else geom.angle = 0;
...
fabric.BeeRect = fabric.util.createClass(fabric.Rect, {
  ...
  _frame2local: function (posFrame) {
    ...
    var posLocal = {
      x:
        (posFrame.x - this.cx) * Math.cos(angle) +
        (posFrame.y - this.cy) * Math.sin(angle),
      y:
        -(posFrame.x - this.cx) * Math.sin(angle) +
...
function updateObsActivityFromForm(obs) {
  ...
  if (logging.guiEvents) console.log("updateObsActivityFromForm: obs=", obs);
}
...
function identify(ctx, rect, radius) {
  // old prototype: obs, x,y, color){

  var color;
  if (rect.status === "new") color = "green";
  else if (rect.status === "db") color = "yellow";
  else color = "red"; //problem
...
function plotTracks(ctx) {
  ...
  setColor = function (f) {
    if (f <= F) {
      color =
        "rgba(255,0,128," + (1 - Math.abs((f - F) / (fmin - F - 1))) + ")";
      //ctx.strokeStyle = "rgba(255,0,0, 0.5)"
    } else {
      color =
        "rgba(0,128,128," + (1 - Math.abs((f - F) / (fmax - F + 1))) + ")";
    }
    return color;
  };
  ...
  for (let id of ids) {
    // For each valid bee ID, create a track for it
    ...
    for (let f = fmin + 1; f <= fmax; f++) {
      ...
      ctx.lineWidth = 1;
      if (z) ctx.setLineDash([]);
      else ctx.setLineDash([10, 10]);
      if (isSelected) {
...
function plotTag(ctx, tag, color, flags) {
  ...
  if (color === undefined) {
    if (tag.hamming == 0) color = "#ff0000";
    else if (tag.hamming == 2) color = "#ff6000";
    else color = "#ffb000";
  }
    ...
    if (tag.frame == getCurrentFrame()) {
      if (showTagsOrientation) {
        ...
        if (overlay.tagMarker == "quad" && tag.p) {
          ...
          ctx.strokeStyle = "#00FF00";
          ctx.lineWidth = 2;
        } else if (overlay.tagMarker == "circle") {
          // 'round'
...
function plotTagOrientation(ctx, tag, color) {
  ...
  if (typeof dir !== "undefined") {
    ...
    ctx.lineTo(
      pt.x + dir[0] * L1 + dir[1] * W1,
      pt.y + dir[1] * L1 - dir[0] * W1
    );
    ctx.strokeStyle = color;
    if (tag.hamming > 2) ctx.setLineDash([4, 4]);
    ctx.stroke();
    ctx.restore();
  }
}
  ...
  if (showTagsTracks) {
    ...
    for (var id in ttags) {
      ...
      for (let fs in tagline) {
        ...
        if (p0 != null && Number(p1.frame) - Number(p0.frame) <= 40) {
          ctx.save();
          ctx.beginPath();
        ...
        else plotTag(ctx, tag, color, { id: false, radius: tProx(f) + 1 });
      }
    }
  }
  ...
  if (showSelectedTagsTracks) {
    ...
    if (typeof defaultSelectedBee !== "undefined") {
      ...
      for (let f = fmin; f <= fmax; f++) {
        ...
        let ii = -1;
        for (let i in tags) {
          if (tags[i].id == defaultSelectedBee) {
            ii = i;
            break;
          }
        }
        ...
        let tag = tags[ii];
        {
          ...
          if (
            ...
            if (
              tag.hamming < 1000 &&
              Number(p1.frame) - Number(p0[tag.id].frame) <= 1
            ) {
              ctx.setLineDash([]);
              ctx.lineWidth = 2;
              color2 = setColor2(f);
...
function onTrackWindowChanged(event) {
  ...
  if (trackDir == "Bidirectional") {
    ...
    console.log(
      "onTrackWindowChanged: ERROR, trackDir unknown trackDir=",
      trackDir
    );
  }
...
function focusTrackWindow() {
  ...
  let f = getCurrentFrame();
  axes.xdomainFocus([
    f - overlay.trackWindow.backward,
...
function setFocusTrackWindow(flag) {
  ...
  else $(overlay).off("trackWindow:change", focusTrackWindow);
...
function onROITextChanged() {
  ...
  if (roi.length != 4) {
    console.log(
      "onROIChanged: ERROR, ROI should have 4 coordinates, left,top,right,bottom"
    );
    return;
  }
  ROI = {
...
function plotROI(ctx) {
  ...
  ctx.fillRect(
    R2.left + R2.width,
    R2.top,
    overlay.canvas.width - R2.left - R2.width,
    R2.height
  );
  ctx.fillRect(
    0,
    ...
    overlay.canvas.height - R2.top - R2.height
  );
  ctx.restore();
}
...
function excludeRectsChanged() {
  ...
  $("#excludeRectDeleteButton").toggleClass(
    "disabled",
    !(excludeRectSelection >= 0)
  );
  onTagsParametersChanged();
  refreshChronogram();
  videoControl.refresh();
  ...
  $("#dialog-form-excludeRects").dialog({
    ...
    buttons: {
      Ok: function () {
        ...
        if (logging.zoomOverlay) console.log("excludeRects = ", excludeRects);
...
tagsDMSampleFilter = function (tag) {
  return true;
};
tagsHammingSampleFilter = function (tag) {
  return true;
};
...
function refreshTagsParameters() {
  ...
  if (minDM > 0) {
    tagsDMSampleFilter = function (tag) {
      return !tagsHaveDM || tag.dm >= minDM;
    };
  } else {
    tagsDMSampleFilter = function (tag) {
      return true;
...
function chronoFilter(mode) {
  if (mode == "H0") {
    tagsHammingSampleFilter = function (tag) {
      return tag.hamming == 0;
    };
    onTagsParametersChanged();
  }
  if (mode == "H1") {
    tagsHammingSampleFilter = function (tag) {
      return tag.hamming <= 1;
    };
    onTagsParametersChanged();
  }
  if (mode == "H2") {
    tagsHammingSampleFilter = function (tag) {
      return tag.hamming <= 2;
    };
    onTagsParametersChanged();
  }
  if (mode == "Hall") {
...
function newRectForCurrentTag() {
  ...
  if (tag == null) {
    printMessage(
      "Current tag not visible in this frame. Try Next or Prev event",
      "red"
    );
    return;
  }
...
function onMouseDown_predict(option) {
  ...
  if (logging.mouseEvents) {
    console.log(
      "onMouseDown_predict: predictId         --> prediction=",
      prediction
    );
    console.log(
      "onMouseDown_predict: predictIdFromTags --> predictionTag=",
      predictionTag
    );
  }
  ...
  if (predictionTag.id !== undefined) {
    ...
    if (prediction.obs && prediction.id == tag.id) {
      ...
      if (logging.mouseEvents)
        console.log(
          "onMouseDown_predict: copying rect from tag ",
          tag,
          " and obs ",
          obs
        );
    ...
    if (
      id == null ||
      getObsHandle(getCurrentFrame(), id, false) !== undefined
    ) {
      id = prediction.id;
    }
    ...
    if (logging.mouseEvents)
      console.log(
        "onMouseDown_predict: did not find tag or event, created new rect with default size ",
        rect
      );
  }
  rect.setCoords();
...
function onMouseDown_interactiveRect(option) {
  ...
  var onMouseUp_interactiveRect = function (e) {
    if (logging.mouseEvents) console.log("onMouseUp_interactiveRect: e=", e);
    ...
    if (logging.mouseEvents)
      console.log(
        "onMouseUp_interactiveRect: rect=",
        rect,
        "active=",
        overlay.canvas1.getActiveObject()
      );
    if (activeObject.validated) {
      ...
      if (logging.mouseEvents)
        console.log(
          "onMouseUp: removing non validated activeObject=",
          activeObject
        );
      deselectBee();
    }
...
function onMouseDown_selectMultiframe(option) {
  ...
  if (!clickMultiframe) {
    ...
    if (showObsTracks) {
      tmp = predictIdFromObsMultiframe(
        [
          ...
          getCurrentFrame() + overlay.trackWindow.forward,
        ],
        pt
      );
      ...
      if (tmp.id != null) {
        console.log(
          "onMouseDown_selectMultiframe: found Obs id=",
          tmp.id,
          "frame=",
          tmp.frame
        );
    ...
    if (!done && (showTagsTracks || showSelectedTagsTracks)) {
      ...
      if (tmp.id != null) {
        console.log(
          "onMouseDown_selectMultiframe: found Tag id=",
          tmp.id,
          "frame=",
          tmp.frame
        );
...
function onMouseDown_panning(option) {
  ...
  var onMouseMove_panning = function (option) {
    ...
    panning.canvasTransform[4] =
      panning.canvasTransform0[4] - dx * panning.canvasTransform0[0];
    panning.canvasTransform[5] =
      panning.canvasTransform0[5] - dy * panning.canvasTransform0[3];
  ...
  var onMouseUp_panning = function (e) {
    if (logging.mouseEvents) console.log("onMouseUp_panning: e=", e);
...
function onMouseDown(option) {
  if (logging.mouseEvents) console.log("onMouseDown: option=", option);
...
function onMouseWheel_Zooming(option) {
  ...
  if (logging.mouseEvents) console.log("onMouseWheel_Zooming: option=", option);
...
function onMouseUp(option) {
  if (logging.mouseEvents) console.log("onMouseUp: option=", option);
...
function onObjectSelected(option) {
  if (logging.selectionEvents) console.log("onObjectSelected:", option);
  ...
  if (typeof option.target.id != "undefined") {
    if (option.target.id != overlay.getActiveObject().id) {
      console.log(
        "ERROR in onObjectSelected: option.target.id != canvas1.getActiveObject().id",
        option.target.id,
        overlay.getActiveObject().id
      );
    }
...
function onObjectDeselected(option) {
  if (logging.selectionEvents) console.log("onObjectDeselected: ", option);

  if (overlay.lastSelected !== null) {
    if (overlay.lastSelected.status == "new") {
      if (logging.mouseEvents)
        console.log(
          "onObjectDeselected: removing non submitted lastSelected=",
          overlay.lastSelected
        );
