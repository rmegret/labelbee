...
class VideoControl {
  constructor(videoTagId) {
    ...
    this.previewVideo.addEventListener(
      "timeupdate",
      function () {
        videoControl.onPreviewFrameChanged();
      },
      false
    );
  }
  // #MARK ### Play/pause
  playRateInputChanged() {
  ...
  getCurrentVideoTime(format) {
    return this.video2.toMilliseconds() / 1000.0;
  }
  getCurrentRealDate(format) {
  ...
  setPreviewVideoStatus(status) {
    ...
    switch (status) {
      ...
      case "infoloaded":
        ...
        $("#previewVideoStatus").prop(
          "title",
          "Preview video loaded with info"
        );
        break;
    }
  }
  loadPreviewVideo(previewURL) {
    let videoControl = this;
