...
function ZoomOverlay(canvas, canvasOverlay) {
  if (this === window) {
    console.log(
      'ERROR: ZoomOverlay should be created with "new ZoomOverlay()"'
    );
...
function KeypointLabelsFromServerDialog() {
  ...
  this.updateDialog = function (uri) {
    console.log(
      this.dialogName + ': importing labellist from URL "' + this.route + "'..."
    );
    ...
    $.ajax({
      ...
      success: function (json) {
        ...
        for (let item of json.files) {
          html +=
            '<button onclick="zoomOverlay.keypointLabelsFromServerDialog.onClickFile(' +
            "'" +
            item["uri"] +
            "'" +
            ')">' +
        ...
        for (let item of json.dirs) {
          html +=
            '<button onclick="zoomOverlay.keypointLabelsFromServerDialog.onClickDir(' +
            "'" +
            item["uri"] +
            "'" +
            ')">' +
      ...
      error: typesetAjaxError(this.dialogName + ": ERROR", function (html) {
  ...
  this.openDialog = function () {
    ...
    if (!div.length) {
      console.log(
        this.dialogName + ": ERROR, could not find div " + this.divId
      );
      return;
    }
    ...
    div
      .find(".modal-message")
      .html("<div>Loading label list from server. Please wait...</div>");
  ...
  this.onClickFile = function (url) {
    console.log(
      this.dialogName + ": importing label list from URL '" + url + "'..."
    );

    div.find(".modal-message").html("Loading labels from " + url + "...");

    zoomOverlay.keypointLabelsFromServer(url).then(function () {
...
ZoomOverlay.prototype.keypointLabelsFromServer = function (url) {
  ...
  console.log(
    "keypointLabelsFromServer: importing label list from URL '" + url + "'..."
  );
...
ZoomOverlay.prototype.selectionChanged = function () {
  if (logging.zoomOverlay) console.log("ZoomOverlay.selectionChanged");

  if (!this.flagShowZoom) {
    if (logging.zoomOverlay)
      console.log(
        "ZoomOverlay.selectionChanged: flagShowZoom==false. Candeled"
      );
    return;
  }
...
ZoomOverlay.prototype.syncFromTracks = function () {
  if (logging.zoomOverlay) console.log("ZoomOverlay.syncFromTracks");
  var activeObject = overlay.getActiveObject(); // Object of video frame

  if (!activeObject) {
    if (logging.zoomOverlay)
      console.log(
        "ZoomOverlay.syncFromTracks: abort. activeObject=",
        activeObject
      );
    this.canvas1.clear();
    return;
...
ZoomOverlay.prototype.syncToTracks = function () {
  if (logging.zoomOverlay) console.log("ZoomOverlay.syncToTracks");
  var activeObject = overlay.getActiveObject(); // Object of video frame

  if (!activeObject) {
    if (logging.zoomOverlay)
      console.log(
        "ZoomOverlay.syncToTracks: abort. activeObject=",
        activeObject
      );
...
ZoomOverlay.prototype.onMouseDown = function (option) {
  if (logging.zoomOverlay) console.log("ZoomOverlay.onMouseDown", option);
  if (option.target) {
...
ZoomOverlay.prototype.onKeyDown = function (e) {
  if (logging.keyEvents) console.log("ZoomOverlay.onKeyDown: e=", e);
...
ZoomOverlay.prototype.onToggleButtonAutoLabel = function (event) {
  ...
  if (logging.zoomOverlay) console.log(event);
...
ZoomOverlay.prototype.setCanvasSize = function (w, h) {
  ...
  $("#zoomresize")[0].style.width =
    (w + 16 + borderThickness).toString() + "px";
...
ZoomOverlay.prototype.canvas2frame = function (posCanvas) {
  var posFrame = {
    x:
      ((posCanvas.x - this.centerCanvas.x) * Math.cos(this.angle) -
        (posCanvas.y - this.centerCanvas.y) * Math.sin(this.angle)) /
        this.scale +
      this.centerFrame.x,
    y:
      ((posCanvas.x - this.centerCanvas.x) * Math.sin(this.angle) +
...
ZoomOverlay.prototype.frame2canvas = function (posFrame) {
  var posCanvas = {
    x:
      ((posFrame.x - this.centerFrame.x) * Math.cos(this.angle) +
        (posFrame.y - this.centerFrame.y) * Math.sin(this.angle)) *
        this.scale +
      this.centerCanvas.x,
    y:
      (-(posFrame.x - this.centerFrame.x) * Math.sin(this.angle) +
...
ZoomOverlay.prototype.onObjectSelected = function (option) {
  if (logging.zoomOverlay) console.log("ZoomOverlay.onObjectSelected", option);
...
ZoomOverlay.prototype.selectLabel = function (label) {
  ...
  if (!label) {
    if (logging.zoomOverlay) console.log("ZoomOverlay.selectLabel: deselect");
    ...
    if (!rect) {
      if (logging.zoomOverlay)
        console.log(
          "ZoomOverlay.selectLabel: select for insertion label=",
          label
        );
      this.canvas1.deactivateAllWithDispatch();
      this.selected = label;
...
ZoomOverlay.prototype.selectNextLabel = function (currentLabel) {
  if (this.autoLabelDone) {
    if (logging.zoomOverlay)
      console.log(
        "ZoomOverlay.selectNextLabel: autoLabelDone=true. Canceling."
      );
    return;
  }
  ...
  if (logging.zoomOverlay) console.log("ZoomOverlay.selectNextLabel: i=", i);
  var j;
  if (i < 0) {
    j = 0;
  } else {
    ...
    if (j == 0) {
      if (logging.zoomOverlay)
        console.log(
          "ZoomOverlay.selectNextLabel: no more label to choose from. Canceled."
        );
  ...
  if (logging.zoomOverlay) console.log("ZoomOverlay.selectNextLabel: j=", j);
  let label = labels[j];
  this.selectLabel(label);
};
...
ZoomOverlay.prototype.deletePoint = function (rect) {
  if (logging.zoomOverlay) console.log("ZoomOverlay.deletePoint(", rect, ")");
...
ZoomOverlay.prototype.onButtonClickAddRemovePartLabel = function (evt, action) {
  ...
  if (action === "+") {
    ...
    if (logging.zoomOverlay)
      console.log(
        "ZoomOverlay.onButtonClickAddRemoveLabel: new label " +
          label +
          " with default color " +
          c
      );
  ...
  if (action === "c") {
    ...
    c = window.prompt(
      "New color for label '" + label + "' (format 'red' or 'rgb(255,0,0)'",
      currentColor
    );
    if (c == null || c == "") {
      ...
      if (logging.zoomOverlay)
        console.log(
          "ZoomOverlay.onButtonClickAddRemoveLabel: label " +
            label +
            " has new color " +
            c
        );
...
ZoomOverlay.prototype.redraw = function () {
  ...
  if (logging.zoomOverlay) console.log("ZoomOverlay.redraw:");
  ...
  this.canvas1
    .getContext("2d")
    .clearRect(0, 0, this.canvas1.width, this.canvas1.height);
...
function drawTagBin(tagbin, n, context, x, y, sx, sy, margin, inv) {
  ...
  for (var x2 = 0; x2 < n; ++x2) {
    for (var y2 = 0; y2 < n; ++y2) {
      var i = y2 * n + x2;
      if (tagbin[i] ^ (inv == 0)) context.fillStyle = "black";
...
ZoomOverlay.prototype.updateTagView = function (tag) {
    ...
    if (nearest) {
      for (var y = ymin; y < ymax; y++) {
        for (var x = xmin; x < xmax; x++) {
          ...
          let x2 = Math.round(dstPt[0]),
            y2 = Math.round(dstPt[1]);
      ...
      for (var y = ymin; y < ymax; y++) {
        for (var x = xmin; x < xmax; x++) {
          ...
          dst_data[(x + y * S) * 4] =
            (1 - a) *
              ((1 - b) * src_data[(x2 + y2 * S2) * 4] +
                b * src_data[(x2 + (y2 + 1) * S2) * 4]) +
            a *
              ((1 - b) * src_data[(x2 + 1 + y2 * S2) * 4] +
                b * src_data[(x2 + 1 + (y2 + 1) * S2) * 4]);
          dst_data[(x + y * S) * 4 + 1] =
            (1 - a) *
              ((1 - b) * src_data[(x2 + y2 * S2) * 4 + 1] +
                b * src_data[(x2 + (y2 + 1) * S2) * 4 + 1]) +
            a *
              ((1 - b) * src_data[(x2 + 1 + y2 * S2) * 4 + 1] +
                b * src_data[(x2 + 1 + (y2 + 1) * S2) * 4 + 1]);
          dst_data[(x + y * S) * 4 + 2] =
            (1 - a) *
              ((1 - b) * src_data[(x2 + y2 * S2) * 4 + 2] +
                b * src_data[(x2 + (y2 + 1) * S2) * 4 + 2]) +
            a *
              ((1 - b) * src_data[(x2 + 1 + y2 * S2) * 4 + 2] +
  ...
  this.currentTagBin = extractTagBin(
    ctx,
    npix - 2,
    pixsize / 2,
    pixsize / 2,
...
ZoomOverlay.prototype.updateDistractors = function () {
  ...
  if (!this.flagShowDistractors) return;
  ...
  if (zoomOverlay.flagShowAlternateHamming && !!zoomOverlay.hammingMatrix) {
    ...
    for (let H in hammingLists) {
      ...
      for (let id of ids) {
        ...
        S +=
          '<div class="alternateTag">' +
          id +
          '<br><img class="alternateTag pixelated" src="' +
          url +
          '" onclick="zoomOverlay.onClickAlternateTag(' +
          id +
          ')"/></div>';
      }
    }
  ...
  if (
    zoomOverlay.flagShowAlternateHamming2 &&
    !!zoomOverlay.tagbin &&
    zoomOverlay.currentTagBin
  ) {
    //let hammingLists = zoomOverlay.hammingMatrix[tag.id]

    var hammingLists = findAllHamming(
      zoomOverlay.currentTagBin,
      zoomOverlay.tagbin,
      5
    );
...
function getCoveredTags(obs) {
  ...
  let tracks = flatTracksAll.filter(function (element) {
...
ZoomOverlay.prototype.bakeTagFix = function (obs) {
  ...
  for (k in tags) {
    ...
    if (obs.fix.newid == "falsealarm") {
      tag.fix.newid = obs.fix.newid;
      if (tag.fix.oldid == undefined) tag.fix.oldid = tag.id;
...
ZoomOverlay.prototype.deleteFalsealarmTags = function () {
  ...
  for (let F in Tags) {
    ...
    Tags[F].tags = filter(Tags[F].tags, (tag) => tag.id != "falsealarm");
...
ZoomOverlay.prototype.refreshZoom = function () {
  if (!this.flagShowZoom || !$("#zoom").is(":visible")) {
    if (logging.zoomOverlay)
      console.log(
        "ZoomOverlay.refreshZoom: zoom not visible or not activated. refresh canceled."
      );
    return;
  }

  if (logging.zoomOverlay) console.log("ZoomOverlay.refreshZoom");
...
ZoomOverlay.prototype.tagImgURL = function (id) {
  ...
  for (var i = 0; i < padding - N; i++) paddedID = "0" + paddedID;
...
ZoomOverlay.prototype.refreshInfo = function () {
  ...
  if (tag) {
    let str = "id=" + tag.id + " H=" + tag.hamming + " dm=" + tag.dm;
    if (tag.fix) {
